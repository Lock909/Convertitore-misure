...
def calcola_ohm(ricerca, input_1, input_2):
    if ricerca == "Tensione": 
        ...
        return f"Corrente (I): {input_1 / input_2:.4f} A" if input_2 != 0 else "Errore: R=0"
    else: 
        return f"Resistenza (R): {input_1 / input_2:.4f} \u03a9" if input_2 != 0 else "Errore: I=0"

def ottieni_sezioni():
    # Questa è la funzione che mancava nel tuo file e bloccava la pagina!
...
def calcola_caduta_avanzata(materiale, isolante, posa, fasi, amp, metri, sez, cos_phi):
    ...
    if "molto gravosa" in posa.lower() or "a1" in posa.lower() or "a2" in posa.lower():
        temp_lavoro = temp_regime
    elif "ventilazione" in posa.lower() or "metodo e" in posa.lower() or "metodo f" in posa.lower() or "metodo g" in posa.lower():
