...
def calcola_potenza_e_corrente(sistema, volt, ampere, watt, cos_phi, calcola_cosa):
    rad = math.sqrt(3) if sistema == "Trifase" else 1.0
    c_phi = 1.0 if sistema == "DC" else cos_phi

    if calcola_cosa == "Estrai da Volt e Ampere":
        p_attiva = volt * ampere * c_phi * rad
        p_apparente = volt * ampere * rad
        p_reattiva = math.sqrt(max(0.0, (p_apparente**2) - (p_attiva**2))) if sistema != "DC" else 0.0
        return {"W": p_attiva, "kW": p_attiva / 1000.0, "VA": p_apparente, "kVA": p_apparente / 1000.0, "VAR": p_reattiva, "kVAR": p_reattiva / 1000.0, "HP": p_attiva / 745.7}
    else:
        denominatore = volt * c_phi * rad
        if denominatore == 0: return None
        ampere_calc = watt / denominatore
        return {"A": ampere_calc, "VA": volt * ampere_calc * rad, "kVA": (volt * ampere_calc * rad) / 1000.0, "HP": watt / 745.7}
...
def calcola_caduta_avanzata(materiale, isolante, posa, fasi, amp, metri, sez, cos_phi, temp_amb, iz_nominale, num_circuiti):
    ...
    tabella_k1 = {10: 1.22 if temp_regime == 70 else 1.15, 15: 1.17 if temp_regime == 70 else 1.11, 20: 1.12 if temp_regime == 70 else 1.07, 25: 1.06 if temp_regime == 70 else 1.04, 30: 1.00 if temp_regime == 70 else 1.00, 35: 0.94 if temp_regime == 70 else 0.96, 40: 0.87 if temp_regime == 70 else 0.91, 45: 0.79 if temp_regime == 70 else 0.87, 50: 0.71 if temp_regime == 70 else 0.82, 55: 0.61 if temp_regime == 70 else 0.76, 60: 0.50 if temp_regime == 70 else 0.71}
    ...
    temp_lavoro = min(temp_regime, temp_amb + (temp_regime - temp_amb) * (tasso_utilizzo ** 2))
    
    rho_t = rho * (1.0 + 0.004 * (temp_lavoro - 20.0))
    z_f = ((rho_t / sez) * 1000.0 * cos_phi) + (0.08 * math.sqrt(1.0 - cos_phi**2))
    dv = ((2.0 if fasi == "Monofase" else math.sqrt(3)) * amp * (metri / 1000.0) * z_f)
